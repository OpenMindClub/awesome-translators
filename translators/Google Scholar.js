{
	...
	"creator": "啊哈船长<714763790@qq.com>,Simon Kornblith, Frank Bennett, Aurimas Vinckevicius",
	...
	"lastUpdated": "2021-04-08 03:46:10"
}
...
function scrapeIds(doc, ids) {
	...
	for (let i=0; i<ids.length; i++) {
		// 提取单个item的点击url
		let urlList = ZU.xpath(doc, '//div[@class="gs_r gs_or gs_scl"]/div[@class="gs_ri"]'	)
		let url = ""
		if(urlList.length>0){
			url = urlList[i].innerHTML
			url = url.match(/href="(.+?)"/g)[0]
			url.match(/href="(.+?)"/g) // 这里有点神奇
			url = RegExp.$1
		}
		...
		// 获取引用数
		let referenceList = ZU.xpathText(doc, '//a[contains(text(),"Cited") or contains(text(),"被引用次数：")]').split(",")
		reference = referenceList[i]
		reference = reference.match(/\d+/g)[0]+" 📅"+nowDate
		...
		ZU.doGet(citeUrl, function(citePage) {
			...
			ZU.doGet(bibUrl, function(bibtex) {
				...
				translator.setHandler("itemDone", function(obj, item) {
					...
					// Attach linked page as snapshotLink if available
					...
					item.publisher = "" // 这里可能会污染extra字段,在这里取消掉
