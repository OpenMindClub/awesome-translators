...
async function getTranslatorsToTest() {
	const translatorFilenames = process.argv[2]
		.split("\n")
	...
	for (const translatorFilename of translatorFilenames) {
		let translatorInfo =
			translatorServer.filenameToTranslator[translatorFilename].metadata;
...
You may want to consider adding '[ci skip]' in the commit message.`
	...
	console.log(
		`Will run tests for translators ${JSON.stringify(
			Array.from(toTestTranslatorNames)
		)}`
	);
	...
	for (let translatorID in results) {
		...
		console.log(
			chalk.bold(
				chalk.bgWhite(
					chalk.black(
						`Beginning Tests for ${translatorID}: ${translatorResults.label}`
					)
				)
			)
		);
		...
		for (let line of output) {
			if (
				line.match(/^TranslatorTester: Running [^T]*Test [0-9]*$/) ||
...
(async function () {
	...
	try {
		...
		await driver.wait(
			until.elementLocated({ id: "extensions-value-btn" }),
			60 * 1000
		);
		...
		await new Promise((resolve) =>
			setTimeout(() => resolve(driver.get(testUrl)), 500)
		);
		await driver.wait(
			until.elementLocated({ id: "translator-tests-complete" }),
			30 * 60 * 1000
		);
