	...
	create: function (context) {
		return {
			Program: function (node) {
				...
				const declaration = node.body.find(
					(node) =>
				...
				for (const testCase of translator.testCases.parsed) {
					caseNo += 1;
					const prefix = `test case${
						testCases[caseNo] ? "" : " " + (caseNo + 1)
					}`;
					const loc = testCases[caseNo]
						? testCases[caseNo].loc.start
						: { start: { line: translator.testCases.start, column: 1 } };
					...
					if (
						!(
							Array.isArray(testCase.items) ||
							(testCase.type === "web" && testCase.items === "multiple")
						)
					) {
						context.report({
					...
					if (["import", "search"].includes(testCase.type) && !testCase.input) {
						...
						if (
							!Object.keys(testCase.input).every((key) =>
								expected.includes(key)
							)
						) {
							let invalidKey = Object.keys(testCase.input).find(
								(key) => !expected.includes(key)
							);
							context.report({
								message: `${prefix} of type "${
									testCase.type
								}" has invalid search term '${invalidKey}' - expected one of ${expected.join(
									", "
								)}`,
								loc,
							});
					...
					if (Array.isArray(testCase.items)) {
						let itemsNode = testCases[caseNo].properties.find(
							(prop) => prop.key.type == "Literal" && prop.key.value == "items"
						).value;
						for (let [itemIndex, item] of testCase.items.entries()) {
							...
							for (let [creatorIndex, creator] of item.creators.entries()) {
								let creatorLoc = itemNode.properties.find(
									(prop) =>
										prop.key.type == "Literal" && prop.key.value == "creators"
								).value.elements[creatorIndex].loc;

								if (
									creator.fieldMode !== undefined &&
									creator.fieldMode !== 1
								) {
									context.report({
									...
									context.report({
										message:
											"creator with fieldMode == 1 should have lastName and no firstName",
										loc: creatorLoc,
									});
