			...
			Program: function (node) {
				...
				for (const comment of context.getSourceCode().getAllComments()) {
					...
					if (comment.value.includes(options.mustMatch)) {
						if (
							firstComment ||
							comment.loc.start.line > header.followingStatement.start.line
						) {
							context.report({
								loc: comment.loc,
				...
				const templateFile = fs.existsSync(options.templateFile)
					...
					: path.resolve(
							path.join(findRoot(context.getFilename()), options.templateFile)
					  );
				if (!fs.existsSync(templateFile))
					throw new Error(`cannot find ${templateFile}`);
				const template = fs.readFileSync(templateFile, "utf-8");

				copyright = {
					holder: header.properties.creator
						? header.properties.creator.value
						: null,
					period: `${new Date().getFullYear()}`,
				};
				if (header.properties.lastUpdated) {
					const year = header.properties.lastUpdated.value.split("-")[0] || "";
					if (year && year !== copyright.period)
						copyright.period = `${year}-${copyright.period}`;
				}
				const licenseText =
					"\n\n" +
				...
				context.report({
					...
					fix:
						firstComment && firstComment.type === "Block"
							? undefined
							: (fixer) =>
									fixer.insertTextBefore(
										header.followingStatement,
										licenseText
									),
				});
			},
