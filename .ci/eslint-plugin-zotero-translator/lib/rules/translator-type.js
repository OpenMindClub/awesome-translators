	...
	create: function (context) {
		return {
			Program: function (node) {
				...
				if (browserSupportNode && !(translatorType & type.web))
					context.report(
						browserSupportNode,
						`browserSupport set, but translatorType (${translatorType}) does not include web (${type.web})`
					);
				...
				for (const name of [
					...
					if (handler && !(translatorType & bit)) {
						context.report(
							handler,
							`${name} present, but translatorType (${translatorType}) does not specify ${mode} (${bit})`
						);
					}
					if (!handler && translatorType & bit) {
						context.report(
							translatorTypeNode,
							`translatorType specifies ${mode} (${bit}), but no ${name} present`
						);
					}
				}
			},
