...
const repo = path.resolve(
	findRoot(__dirname, (dir) => fs.existsSync(path.resolve(dir, ".git")))
);
...
function jsonParseWithErrorInfo(raw, source) {
	...
	try {
		...
		target.error = {
			message: err.message,
			line:
				source.substring(0, source.indexOf(raw)).split("\n").length + // start of raw JSON
...
function strip(source) {
	...
	return (
		tryFormatJSON(header) +
		(prefix ? "\n\n" + prefix.replace(/^\s*/, "") : "") +
		code +
		(testCasesPrefix || "") +
		tryFormatJSON(testCasesVar || "") +
		(testCases || "")
	);
}
...
class Cache {
	constructor() {
		...
		for (const translator of fs.readdirSync(this.repo)) {
			...
			this.decorated[path.basename(translator)] = decorate(
				fs.readFileSync(path.join(repo, translator), "utf-8")
			);
		}

		const branch = exec("git rev-parse --abbrev-ref HEAD").trim();

		const hasUpstream = exec("git remote -v")
			.split("\n")
			.map((line) => line.trim())
			.includes("upstream\thttps://github.com/zotero/translators.git");
		// branch to compare lastUpdated against -- assume that if have upstream/main, you want to compare against that
		const main = hasUpstream ? "upstream/main" : "main";

		this.lastUpdated = {};
		if (branch !== main) {
			// `git diff --name-status ${main}` will fetch the names of the files that have changed against `${main}`
			for (const file of exec(`git diff --name-status ${main}`).split("\n")) {
			...
			/*
				We do a `git grep '"lastUpdated"' ${main} *.js` to get the
				`lastUpdated` values from the `${main}` branch. For files that are
			...
			*/
			for (const lu of exec(`git grep '"lastUpdated"' ${main} *.js`).split(
				"\n"
			)) {
				const m = lu.match(
					/^[a-z/]+:([^:]+):\s*"lastUpdated"\s*:\s*"([-0-9: ]+)"/
				);
				if (!m) continue;
				const translator = m[1];
				if (this.decorated[translator] && this.decorated[translator].modified)
					this.decorated[translator].lastUpdated = m[2];
			}
		}
	}
	...
	get(filename) {
		...
		if (
			!this.decorated[basename] &&
			path.dirname(path.resolve(filename)) !== this.repo
		)
			this.decorated[basename] = {};
	...
	conflicts(filename, translatorID) {
		...
		for (const [otherFilename, otherHeader] of Object.entries(this.decorated)) {
			if (
				...
				otherHeader.translatorID === translatorID
			) {
...
function getHeaderFromAST(programNode) {
	...
	if (
		...
		declaration.declarations[0].id.name !== headerVar
	)
		return {};
	...
	return {
		...
		body,
		properties,
		followingStatement: programNode.body[1],
	};
}
