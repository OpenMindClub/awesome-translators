...
argv
	...
	.option(
		"--dump-decorated [file]",
		"Dump decorated translator to file for inspection"
	)
	.parse(process.argv);
...
for (const target of argv.args) {
	...
	const files = fs.lstatSync(target).isDirectory()
		? find(target, [findIgnore])
		: [target];
...
if (sources.javascripts.length) {
	...
	if (argv.fix) {
		for (const result of report.results) {
			if (
				result.messages.find((msg) => msg.ruleId === "notice/notice" && msg.fix)
			) {
...
for (const translator of sources.translators) {
	if (argv.dumpDecorated)
		fs.writeFileSync(argv.dumpDecorated, translator.source, "utf-8");
		...
		for (const result of report.results) {
			if (result.output) {
				try {
					fs.writeFileSync(
						result.filePath,
						translators.strip(result.output),
						"utf-8"
					);
				} catch (err) {
